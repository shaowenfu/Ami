...
export const AMI_ACCESS_TOKEN = env.EXPO_PUBLIC_AMI_ACCESS_TOKEN?.trim() ?? '';
export const AMI_CURRENT_USER_ID = env.EXPO_PUBLIC_AMI_USER_ID?.trim() ?? '';
export const AMI_DEFAULT_SPACE_ID = env.EXPO_PUBLIC_AMI_SPACE_ID?.trim() ?? '';

export function isBackendChatConfigured() {
  return Boolean(AMI_API_BASE_URL && AMI_ACCESS_TOKEN && AMI_DEFAULT_SPACE_ID);
}

export function isBackendApiConfigured() {
  return Boolean(AMI_API_BASE_URL && AMI_ACCESS_TOKEN);
}
