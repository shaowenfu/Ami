...
- 2026-05-21：启动 F2/F3 前端接入。新增 Expo 端 API client、Space/Message API 封装、fetch-streaming SSE parser、后端聊天 store，并让聊天页在配置 `EXPO_PUBLIC_AMI_SPACE_ID` / `EXPO_PUBLIC_AMI_ACCESS_TOKEN` 后可走真实后端；未配置时继续使用现有 mock demo。
- 2026-05-21：启动 F1 全局层。新增 `/spaces` 空间概览页与 `useSpaceStore`，应用根路径先进入空间列表；选择空间后进入聊天页，聊天页优先使用当前选中的 space。
- 2026-05-21：继续 F1/F2 全局层。新增 `/space/[spaceId]/` 动态空间层 tab wrapper，空间列表现在跳转到明确的 space 路径；新增 `/spaces/create` 与 `/spaces/invitations`，接入创建邀请、邀请收件箱、接受/拒绝邀请 API。
- 2026-05-21：完成本阶段收尾。前端 SSE client 已支持断线重连、服务端 `retry` 与 `Last-Event-ID` 预留；聊天页已对私聊/群聊做标题、说明、输入区、气泡色和群聊发送提示的强视觉区分。

## 阶段收尾状态

本计划第一阶段的“应用骨架与基础功能”已经形成端到端闭环：

- 后端：Space / Invitation / Message / SSE / 基础 Ami 回复 / Mem0 Adapter / 轻量记忆消化已落地。
- 前端：全局空间入口、创建邀请、邀请信箱、动态空间层 tabs、消息 API、SSE client、后端聊天 store 已落地。
- 隐私边界：后端按 `PRIVATE:<user_id>` / `SHARED` 解析 room scope，SSE 私聊事件只推当前用户，Mem0 召回按 room scope 使用强过滤器。
- 降级策略：Mem0 未启用时退化为 Mongo 最近消息；前端未配置后端 token 时保留 mock demo。
- 验证：已通过后端 `compileall`、前端 TypeScript 检查、`git diff --check`，并完成 Expo Web `/spaces` 与 `/space/demo-space/chat` 冒烟截图验证。

剩余工作不再属于“骨架收尾”，建议进入下一阶段专项：

- Auth 页面与 refresh token 流程。
- Redis Stream / sse_event_log 断线续传服务端实现。
- 记忆 digest checkpoint、批量摘要、删除映射和隐私测试。
- 将旧 `(tabs)` 兼容入口完全迁移到 `/space/[spaceId]` 后再移除。
- 真实端到端测试环境：Mongo、Redis、LLM、Mem0 API key、两位测试用户。
...
当前状态：已新增 `/spaces` 作为第一版全局层入口，根路径重定向到空间概览；已新增 `/space/[spaceId]/` 动态空间层 tab wrapper，Chat / Tools / Moments / Profile 可以挂在明确的 space 参数下。当前仍保留旧 `(tabs)` 作为兼容入口，后续可在验证稳定后移除或整体迁移。
...
当前状态：已新增前端 API client、Space/Message API 封装、后端聊天 store 与空间 store。第一版使用 `EXPO_PUBLIC_AMI_ACCESS_TOKEN` 作为静态接入 token；`/spaces/create` 已可按手机号创建邀请，`/spaces/invitations` 已可查看并接受/拒绝邀请。refresh token 流程留待 Auth 页面接入后实现。
...
当前状态：已先实现 fetch streaming SSE client，原因是浏览器原生 `EventSource` 无法附带 `Authorization` header，而当前后端 SSE 端点走同一套 Bearer / `X-Auth-Token` 鉴权。客户端已支持断线重连、服务端 `retry` 字段与 `Last-Event-ID` header 预留；后续需要服务端补事件日志/Redis Stream 才能真正按游标续传。
...
当前状态：已完成第一版。聊天页会根据 room mode 切换标题、说明卡片、输入区底色、用户气泡色和输入 placeholder；群聊模式会在输入区上方提示“双方可见 / 进入共享上下文”。
